...
public class UsbAdmin {
	...
	private void setDevice(UsbDevice device)  {
    	if(device!=null) {
	    	...
	    	int j,InterfaceCount=device.getInterfaceCount();
	    	...
	    	for (j=0;j<InterfaceCount;j++) {
	    		...
	    		if(intf.getInterfaceClass()==7)	{
	    			...
	    			for(i = 0;i < UsbEndpointCount;i++)	{
	    				...
	    				if(ep.getDirection() == 0 && ep.getType() ==
	    			...
	    			if (i!=UsbEndpointCount) break;
	    		}
	    	}

	    	if (j == InterfaceCount) {
	    		Log.i(TAG,"Not printer class interface");return;
	    	}
	...
	public boolean GetUsbStatus() {
		if (mConnection==null) return false;else return true;
	}
	...
	public boolean sendCommand(byte [] Content) {
        ...
        synchronized (this)  {
            ...
            if (len<0) {
            	Result=false;Log.i(TAG,"xxx "+len);
            }  else {
            	Result=true;Log.i(TAG,"xxx"+len+"xxxx");
            }
        }
        return Result;
	}
